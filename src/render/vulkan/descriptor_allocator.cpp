...
namespace render::vulkan {
	...
	DescriptorAllocatorPool::DescriptorAllocatorPool(VkDevice device,
													 u32 max_frames) :
		mDevice(device),
	...
	DescriptorAllocatorPool::DescriptorAllocatorPool(
		VkDevice device, std::span<PoolSize> pool_sizes, u32 max_frames) :
		mDevice(device),
		mFrameIndex(0), mMaxFrames(max_frames) {
	...
	DescriptorAllocatorPool::operator=(DescriptorAllocatorPool&& other) noexcept {
		mDevice = other.mDevice;
	...
	DescriptorAllocator&& DescriptorAllocatorPool::get_allocator() {
		...
		if (!found) {
			...
			allocator = create_pool(2000, flags);
		}
		return {this, poolIndex, allocator};
	}
	...
	DescriptorAllocatorPool::create_pool(int count,
		...
		for (auto size : mPoolSizes.sizes) {
			...
			sizes.push_back(
				{size.type, static_cast<u32>(size.multiplier * count)});
	...
	void DescriptorAllocatorPool::set_pool_size_multiplier(VkDescriptorType type,
														   f32 multiplier) {
		auto it = std::find_if(mPoolSizes.sizes.begin(), mPoolSizes.sizes.end(),
